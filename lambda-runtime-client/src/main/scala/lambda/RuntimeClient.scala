...
case class CognitoIdentity(identity_id: String,
...
object EventContext {
  ...
  def apply(headers: Map[String, String]): EventContext = {
    EventContext(
      headers(RequestId.name),
      headers(FunctionArn.name),
      headers(TraceId.name),
      headers(Deadline.name).toLong,
      headers.get(ClientContext.name)
        .flatMap(x => decode[ClientContext](x).fold(_ => None, (context: ClientContext) => Some(context))),
      headers.get(CognitoIdentity.name).flatMap(x => decode[CognitoIdentity](x).fold(_ => None, (identity: CognitoIdentity) => Some(identity)))
    )
  }
}
